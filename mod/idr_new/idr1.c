...
static int idr1_init(void)
{
	...
	idr_init(&idr2);
	for (i = 1; i <= 5; i ++) {
		ret = idr_alloc_cyclic(&idr1, NULL, &handle,
				       start, end + 1, GFP_KERNEL);
		printk(KERN_ALERT "handle = %ld\n", handle);
	}
	idr_remove(&idr1, 3);
/* 	idr_set_cursor(&idr1, 1); */
	ret = idr_alloc_cyclic(&idr1, NULL, &handle,
			       start, end, GFP_KERNEL);
	printk(KERN_ALERT "handle2 = %ld\n", handle);
	...
	ret = idr_alloc(&idr1, NULL, &handle, 0, 1, GFP_KERNEL);
	printk(KERN_ALERT "ret: %d, handle = %ld\n", ret, handle);
