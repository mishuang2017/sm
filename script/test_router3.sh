...
ovs-ofctl add-flow $br "table=1,in_port=2,ip,ct_state=+trk+new,action=ct(nat(src=8.9.10.1-8.9.10.1:5000-50000),commit),mod_dl_src:${MAC_ROUTE},mod_dl_dst:${MAC2},5"
...
#回来流量，已经建立连接，直接发给rep
ovs-ofctl add-flow $br "table=1,in_port=5,ip,ct_state=+trk+est,action=mod_dl_src:${MAC_ROUTE},mod_dl_dst:${MAC1},2"
